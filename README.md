...
A once-a-day summary of AI signal from Reddit and a small set of RSS feeds, distilled by Claude Haiku 4.5 into content angles for an AI-systems consulting personal brand and delivered to Telegram. Scheduling runs on GitHub Actions cron.
...
- **Sources, thresholds, model, lookback window, top-N**: edit the `CONFIG` block at the top of `main.py` (`RSS_FEEDS`, `REDDIT_SUBS`, `REDDIT_SEARCHES`, `LOOKBACK_HOURS`, `TOP_N_STORIES`, `MIN_REDDIT_SCORE`, `MIN_SEARCH_SCORE`, `MODEL`).
